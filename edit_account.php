...
if (isset($_POST['update'])) {
    ...
    if (!preg_match('/^9[0-9]{9}$/', $phone)) {
        $msg = "Phone must be exactly 10 digits starting with 9 (e.g., 98XXXXXXXX)!";
        $msgType = "error";
    }
    ...
    elseif (!empty($password)) {
        ...
        if (strlen($password) < 5) {
            ...
            // Valid password - hash it before storing
            $hashed_password = password_hash($password, PASSWORD_DEFAULT);
            $update_sql = "UPDATE user SET fullname='$fullname', email='$email', phone='$phone', password='$hashed_password' WHERE username='$username'";
...
<head>
    ...
    <title>Edit Account - BusGo</title>
...
<body>
    ...
    <div class="container">
        <div class="card">
            <div class="card-header">
                <h2><i class="fas fa-user-cog"></i> Edit Account</h2>
            ...
            <form id="editForm" method="POST">
                ...
                <div class="form-group">
                    <label for="phone"><i class="fas fa-phone"></i> Phone Number <span
                            style="color:red;">*</span></label>
                    <input type="text" name="phone" id="phone" value="<?php echo htmlspecialchars($user['phone']); ?>"
                        required placeholder="98XXXXXXXX" maxlength="10">
                    <div class="hint" id="phoneHint">
                        <i class="fas fa-info-circle"></i> Must be exactly 10 digits starting with 9 (e.g., 98XXXXXXXX)
                    </div>
                </div>

                <!-- Password Field -->
                <div class="form-group">
                    <label for="password"><i class="fas fa-lock"></i> New Password <span
                            style="color:#6b7a8f;font-weight:400;">(optional)</span></label>
                    <div class="password-wrapper">
                        <input type="password" name="password" id="password"
                            placeholder="Leave blank to keep current password" maxlength="50">
                        <button type="button" class="toggle-password" onclick="togglePassword()">
                            <i class="fas fa-eye" id="toggleIcon"></i>
                        </button>
                    </div>
                    <div class="hint" id="passwordHint">
                        <i class="fas fa-info-circle"></i> At least 5 characters, 1 capital letter, 1 digit, 1 special
                        character
                    </div>
                    <div class="password-requirements" id="passwordRequirements">
                        <span class="req-item unmet" id="reqLength"><i class="fas fa-times-circle"></i>
                        </span>
                        <span class="req-item unmet" id="reqCapital"><i class="fas fa-times-circle"></i> 1 Capital
                            letter</span>
                        <span class="req-item unmet" id="reqDigit"><i class="fas fa-times-circle"></i> 1 Digit</span>
                        <span class="req-item unmet" id="reqSpecial"><i class="fas fa-times-circle"></i> 1 Special
                            char</span>
                    </div>
                </div>
    ...
    <script>
        ...
        document.getElementById('phone').addEventListener('input', function () {
            ...
            if (value.length === 0) {
                hint.innerHTML = '<i class="fas fa-info-circle"></i> Must be exactly 10 digits starting with 9 (e.g., 98XXXXXXXX)';
                hint.className = 'hint';
                this.className = '';
            } else if (isValid) {
                ...
                hint.innerHTML = '<i class="fas fa-exclamation-circle"></i> Invalid! Must be 10 digits starting with 9';
                hint.className = 'hint invalid';
        ...
        document.getElementById('editForm').addEventListener('submit', function (e) {
            ...
            if (!/^9[0-9]{9}$/.test(phone)) {
                e.preventDefault();
                alert("❌ Phone must be exactly 10 digits starting with 9 (e.g., 98XXXXXXXX).");
                return false;
            }
            if (password.length > 0) {
                const reqs = checkPassword(password);
        ...
        <?php if ($updated): ?>
            ...
            setTimeout(() => { document.getElementById('popup').style.display = 'none'; }, 3500);
        <?php endif; ?>
    </script>
